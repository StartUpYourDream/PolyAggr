  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-gray-100">Order Book</h3>
        <div className="text-xs text-gray-500">
          Spread: <span className="text-gray-300">{(spread * 100).toFixed(2)}¢</span>
          {' '}
      ...
      {/* Asks (卖单) */}
      <div className="flex-1 flex flex-col-reverse space-y-reverse space-y-0.5 mb-2">
        {asks.map((ask, index) => {
      ...
      {/* 当前价格 */}
      <div className="py-3 border-y border-dark-600 text-center">
        <div className="text-2xl font-bold text-gray-100 font-mono tabular-nums">
          {(currentPrice * 100).toFixed(1)}¢
        </div>
        <div className="text-xs text-gray-500 mt-0.5">Last Price</div>
      </div>

      {/* Bids (买单) */}
      <div className="flex-1 flex flex-col space-y-0.5 mt-2">
        {bids.map((bid, index) => {
